...
{   
    ...
    public Optional<Order> get(int id) {
        ...
        try {
            ...
            Order order = null;
            while (rs.next()) {
                order = new Order(rs.getInt("Order_ID"), rs.getString("Item_Name"), rs.getString("Options"), rs.getInt("Quantity"), rs.getDouble("UnitPrice"),rs.getDouble("Total"));
            }
    ...
    public void insert(Order order)
    {
        DB db = DB.getInstance();
        try {
            String sql = "INSERT INTO OD_Order(Order_ID,Item_Name,Options,Quantity,UnitPrice,Total) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement stmt = db.getPreparedStatement(sql);
            stmt.setInt(1, order.getID());
            stmt.setString(2, order.getItemName());
            stmt.setString(3, order.getOptions());
    ...
    public void update(Order order) {
        DB db = DB.getInstance();
        try {
            String sql = "UPDATE OD_Order SET Item_Name=?, Options=?, Quantity=?, UnitPrice=?, Total=? WHERE Order_ID=?";
            PreparedStatement stmt = db.getPreparedStatement(sql);
            stmt.setString(1, order.getItemName());
            stmt.setString(2, order.getOptions());
            stmt.setInt(3, order.getQuantity());
            stmt.setDouble(4, order.getUnitPrice());
